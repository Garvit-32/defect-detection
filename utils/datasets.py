...
def random_crop(img, labels, crop_range=(0.3, 1), debug_bol=False):
    ori_img = img.copy()
    ori_labels = labels.copy()
    for itr in range(10):

        height = random.randint(
            int(crop_range[0]*img.shape[0]), int(crop_range[1]*img.shape[0]))
        width = random.randint(
            int(crop_range[0]*img.shape[1]), int(crop_range[1]*img.shape[1]))

        x = random.randint(0, img.shape[1] - width)
        y = random.randint(0, img.shape[0] - height)
        labels[:, 1:] = xywhn2xyxy(
            labels[:, 1:], img.shape[1], img.shape[0]
        )
        new_labels = np.zeros_like(labels)

        img = img[y:y+height, x:x+width]
        for idx, label in enumerate(labels):
            if label[1] >= x and label[3] <= x+width and label[2] >= y and label[4] <= y+height:
                ...
                new_labels[idx, 0] = label[0]
                new_labels[idx, 1] = (
                    (int(label[1]) + int(label[3])) / 2) / img.shape[1]
                new_labels[idx, 2] = (
                    (int(label[2]) + int(label[4])) / 2) / img.shape[0]
                new_labels[idx, 3] = (
                    int(label[3]) - int(label[1])) / img.shape[1]
                new_labels[idx, 4] = (
                    int(label[4]) - int(label[2])) / img.shape[0]
        if np.any(new_labels):
            break
        else:
            img = ori_img.copy()
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            ...
            # if random.random() < hyp["random_crop"]:
            #     img, labels = random_crop(
            #         img, labels, (hyp['random_crop_range_l'], hyp['random_crop_range_u']))
            #     h, w, _ = img.shape
